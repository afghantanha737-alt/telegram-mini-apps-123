...
function getDailyAds(user) {
  ...
  return {
    ...
    lastWatchedAt: user.dailyAds.lastWatchedAt
      ? new Date(user.dailyAds.lastWatchedAt)
      : null,
...
function getCooldownSeconds(dailyAds) {
  const lastWatchedAt = dailyAds.lastWatchedAt
    ? new Date(dailyAds.lastWatchedAt).getTime()
    : 0;

  if (!Number.isFinite(lastWatchedAt) || !lastWatchedAt) {
    return 0;
  }

  return Math.max(
    0,
    Math.ceil((adsCooldownMs - (Date.now() - lastWatchedAt)) / 1000)
  );
}
...
router.get('/reward', async (req, res) => {
  ...
  try {
    return res.json({
      success: true,
      ...(await awardAd(telegramId, eventId))
    });
  } catch (error) {
